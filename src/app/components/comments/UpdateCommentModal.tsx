...
const UpdateCommentModal = ({ setOpen, text, commentId }: UpdateCommentModalProps) => {
    ...
    const formSubmitHandler = async (e: FormEvent) => {
        ...
        try {
            await axios.put(`${LOCAL_DOMAIN}/comments/${commentId}`, { text: updatedText }
    ...
    return (
        <div className="fixed top-0 left-0 bottom-0 right-0 z-10 bg-black bg-opacity-40 flex items-center justify-center">
            <div className="w-11/12 lg:w-2/4 bg-white rounded-lg p-3">
                <div className="flex justify-end items-start mb-5"><IoMdCloseCircleOutline onClick={() => setOpen(false)}
                    className="text-red-500 cursor-pointer text-3xl" /></div>
                <form onSubmit={formSubmitHandler}>
                    <input type="text" placeholder="Edit your comment" className="text-xl rounded-lg p-2 w-full bg-white mb-2"
                        value={updatedText}
